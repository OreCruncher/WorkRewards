...
public class WorldCommand implements CommandExecutor
{
    ...
    public WorldCommand(WorkRewards plugin)
    {
        this.plugin = plugin;
    }
        ...
        if (sender instanceof ConsoleCommandSender || (player != null && player.hasPermission("workrewards.config")))
        {
            if (args.length == 2)
            {
                try
                {
                    ...
                    plugin.rewards.setWorld(key, value);
                    plugin.rewards.save(plugin.getConfig());
                    plugin.saveConfig();
                    sender.sendMessage(ChatColor.GOLD + "The world " + key + " has been set to a modifier of " + value);
                    return true;
                }
                catch (NumberFormatException ex)
                {
                    sender.sendMessage(ChatColor.RED + "The world modifier has to be valid decimal amount!");
                }
            }
        }
